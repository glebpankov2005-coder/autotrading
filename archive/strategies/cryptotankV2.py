...
class cryptotankV2(IStrategy):
    ...
    use_trend_filter = BooleanParameter(default=True, space="buy", optimize=False)  # NEW: regime filter
   ...
   # This is called when placing the initial order (opening trade)
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        ref = f'reference_ma_{self.reference_ma_length.value}'
        cond = (df['min'] < self.buy_ma_slope.value) & (df['volume'] > 0)
        if self.use_trend_filter.value:
            cond &= (df['close'] > df[ref])   # NEW: only buy dips while price is above the long MA
        df.loc[cond, ['enter_long', 'enter_tag']] = (1, 'SLOPE')

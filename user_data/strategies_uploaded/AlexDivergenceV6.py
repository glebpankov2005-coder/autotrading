...
class Alex_DivergenceV6(IStrategy):
    """
    ...
    # Set True so indicators are not recomputed intra-candle (reduces live signal flicker).
    ...
    # ema200 + divergence lookback need plenty of warmup, otherwise early candles are garbage.
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        pivots = pivot_points(informative, window=2)
        ...
        Alex_DivergenceV6.plot_config = (
            ...
            .config)
        dataframe['div_signal_debug'] = dataframe[resample('total_bullish_divergences_count')]
        dataframe['div_signal_shifted'] = dataframe[resample('total_bullish_divergences_count')].shift(1)
        return dataframe
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Initialize the columns properly
        dataframe.loc[:, 'enter_long'] = 0
        dataframe.loc[:, 'enter_short'] = 0
        dataframe.loc[:, 'enter_tag'] = ''
        
        # Define conditions first
        # NOTE: divergence markers are written at the pivot bar, but a pivot is only
        # confirmed `pivot window` (=5) candles later. We shift by 5 so entries can only
        # fire once the divergence is actually knowable — removes the hidden lookahead.
        long_conditionhq = (
            (dataframe[resample('total_bullish_divergences')].shift(2) > 0) &
            (
                (dataframe[resample('low')] <= dataframe[resample('kc_lowerband')]) |
                (dataframe[resample('close')] <= dataframe[resample('kc_lowerband')]) |
        ...
        short_conditionhq = (
            (dataframe[resample('total_bearish_divergences')].shift(2) > 0) &
            (
                (dataframe[resample('high')] >= dataframe[resample('kc_upperband')]) |
                (dataframe[resample('close')] >= dataframe[resample('kc_upperband')]) |
                (dataframe[resample('high')] > dataframe[resample('bollinger_upperband')])
            ) &
        ...
        dataframe.loc[long_conditionhq, 'enter_long'] = 1
        dataframe.loc[long_conditionhq, 'enter_tag'] = 'hq_bullish_div_long'
        
        dataframe.loc[short_conditionhq, 'enter_short'] = 1
        dataframe.loc[short_conditionhq, 'enter_tag'] = 'hq_bearish_div_short'
        dataframe.loc[long_condition, 'enter_long'] = 1
        dataframe.loc[long_condition, 'enter_tag'] = 'bullish_div_long'
        
        dataframe.loc[short_condition, 'enter_short'] = 1
        dataframe.loc[short_condition, 'enter_tag'] = 'bearish_div_short'
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        # ===== REVERSAL DIVERGENCE EXIT =====
        # The divergence marker sits at the PIVOT bar, which is `window` candles
        # behind the current bar (pivot needs `window` future candles to confirm).
        # So we must scan the recent window, not just the last bar.
        # exit_div_lookback MUST be >= pivot window + 1.  Pivot window = 5 -> 6.
        exit_div_lookback = 6

        recent = dataframe.iloc[-exit_div_lookback:]
        bull_now = (recent['total_bullish_divergences'] > 0).any()
        bear_now = (recent['total_bearish_divergences'] > 0).any()

        # Opposite divergence appeared -> momentum flipped against the position -> exit
        if trade.is_short and bull_now:
            return 'Exit_Short_Bull_DIV'
        if (not trade.is_short) and bear_now:
            return 'Exit_Long_Bear_DIV'

        # ----- existing ATR take-profit below, unchanged -----
        for i in range(1, len(dataframe['close'])):
            if dataframe.iloc[-i]['date'].to_pydatetime().replace(tzinfo=datetime.timezone.utc) == trade.open_date_utc:
                ...
                atr = entry_candle.get(resample('atr'), entry_candle.get('atr', 0))
                entry_high = entry_candle.get(resample('high'), entry_candle.get('high', 0))
    ...
    for index, row in enumerate(dataframe.itertuples(index=True, name='Pandas')):
        ...
        if bearish_occurence != None:
            ...
            if can_exist:
                ...
                # FIXED: was writing the count/names 30 bars in the past (index-30),
                # inconsistent with the bullish path which writes at index. Now symmetric.
...
def bearish_divergence_finder(dataframe, indicator, high_iterator, index):
    """
    Detects ONLY classic bearish divergence:
    - Price making HIGHER highs (bullish price action)
    - Indicator making LOWER highs (bearish momentum)
    This is a true reversal signal.
    """
    if high_iterator[index] == index:
        ...
        for i in range(current_index-1, current_index-6, -1):
            ...
            # ONLY Classic Bearish Divergence
            # Price higher highs AND indicator lower highs
            if (dataframe['pivot_highs'][current_pivot] > dataframe['pivot_highs'][prev_pivot] and 
                indicator[current_pivot] < indicator[prev_pivot]):
                
                # REMOVED: Hidden divergence pattern
                # (dataframe['pivot_highs'][current_pivot] < dataframe['pivot_highs'][prev_pivot] and 
                #  indicator[current_pivot] > indicator[prev_pivot])
...
def bullish_divergence_finder(dataframe, indicator, low_iterator, index):
    """
    Detects ONLY classic bullish divergence:
    - Price making LOWER lows (bearish price action)
    - Indicator making HIGHER lows (bullish momentum)
    """
    if low_iterator[index] == index:
        ...
        for i in range(current_index-1, current_index-6, -1):
            ...
            # === ONLY CLASSIC BULLISH DIVERGENCE ===
            # Price lower lows AND indicator higher lows
            if (dataframe['pivot_lows'][current_pivot] < dataframe['pivot_lows'][prev_pivot] and 
                indicator[current_pivot] > indicator[prev_pivot]):
                
                # REMOVED THIS LINE (hidden divergence):
                # or (dataframe['pivot_lows'][current_pivot] > dataframe['pivot_lows'][prev_pivot] and 
                #     indicator[current_pivot] < indicator[prev_pivot])
...
def pivot_points(dataframe: DataFrame, window: int = 5, pivot_source: PivotSource = PivotSource.Close) -> DataFrame:
    ...
    # REMOVED: "find last one" provisional-pivot block.
    # It confirmed a pivot at the chart edge using only ONE right-side candle instead of
    # the full `window`. That tentative pivot would disappear once a later candle invalidated
    # it -> the repaint / "vanishing signal" you saw, and it let entries fire on signals that
    # did not yet exist. Only fully-confirmed pivots (window candles each side) are kept now.

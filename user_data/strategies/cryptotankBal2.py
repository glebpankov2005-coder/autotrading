...
class cryptotankBal2(IStrategy):
   ...
   # This is called when placing the initial order (opening trade)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # only block entries in a CONFIRMED deep bear (price below a FALLING 200-EMA)
        dataframe['not_deep_bear'] = ~((dataframe['close'] < dataframe['ema200']) & (dataframe['ema200'] < dataframe['ema200'].shift(72)))
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df.loc[
            (
                ...
                (df['not_deep_bear']) &   # skip only confirmed deep bears
                (df['volume'] > 0)
            ),
    ...
    def populate_exit_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df.loc[
            (
                (qtpylib.crossed_below(df['close'], df['ema200'])) &  # regime break -> bail
                (df['volume'] > 0)
            ),
            ['exit_long', 'exit_tag']] = (1, 'regime break')
        return df

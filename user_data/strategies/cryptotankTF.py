...
class cryptotankTF(IStrategy):
    ...
    # frequency knob: enter/exit when normalised slope z-score crosses +/- k. Lower k = MORE trades.
    k = DecimalParameter(0.5, 3.0, default=1.5, decimals=2, space="buy", optimize=True)
    z_window = 200  # rolling window for slope normalisation (timeframe-agnostic)
   ...
   # This is called when placing the initial order (opening trade)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Normalise slope by its own rolling volatility -> timeframe-agnostic z-score
        _std = dataframe['smooth_ma_slope'].rolling(self.z_window).std()
        dataframe['slope_z'] = dataframe['smooth_ma_slope'] / _std
        l = self.max_length.value
        dataframe['min'] = dataframe['slope_z'].rolling(l).min()
        dataframe['max'] = dataframe['slope_z'].rolling(l).max()
